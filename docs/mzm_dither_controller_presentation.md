...
*   **一阶谐波 (1f)**：$H_1 \propto J_1(m) \sin(2\theta_{err})$ —— 正比于一阶导数（斜率）。
*   **二阶谐波 (2f)**：$H_2 \propto J_2(m) \cos(2\theta_{err})$ —— 正比于二阶导数（曲率）。
...
为了从噪声中精准提取谐波幅度，我们采用离散时间锁相放大原理。对于采样序列 $x[n]$，计算第 $k$ 阶谐波的同相分量 $I_k$与正交分量 $Q_k$：

$$ I_k = \frac{2}{N}\sum_{n=0}^{N-1} x[n]\sin(k\Omega n),\quad Q_k = \frac{2}{N}\sum_{n=0}^{N-1} x[n]\cos(k\Omega n) $$
...
*   **电谱 (Electrical Spectrum)**：
    *   仿真中加入了**热噪声 (Thermal Noise)**、**散粒噪声 (Shot Noise)** 和 **相对强度噪声 (RIN)**。
    *   在频谱上清晰观测到 $f_{dither}$ (10 kHz) 和 $2f_{dither}$ 的信号峰，且信噪比符合实际预期。
    *   **意义**：证实了作为 AI 输入的观测信号是物理真实的。
...
注：从电流幅度 $A_k$ 到功率 $P_{dBm}$ 的转换公式为：
$$ P_{\text{avg}} = \frac{A_k^2}{2}R_{load} \Rightarrow P_{\text{dBm}} = 10\log_{10}(P_{\text{avg}} \cdot 1000) $$
...
2.  **受限学生 (The Student Policy)**：
    *   也就是我们要训练的神经网络。
    *   它**看不见** $V_{real}$，只能看见有噪声的、模糊的导频特征 $\mathbf{x}$。
...
*   **物理引擎调用**：
    ...
    *   利用 PyTorch 批量计算数万个点的 MZM 传输 + FFT 变换。
...
### 7.2 鲁棒性分析
*   **光功率无关**：由于采用了 $I_{DC}$ 归一化，算法对激光器功率波动完全不敏感。
